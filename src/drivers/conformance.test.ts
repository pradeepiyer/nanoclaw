...
function dockerHarness(): Harness {
  ...
  const driver = withSessionEvents(
    new DockerSessionDriver({ ...FIXTURE_POLICY, cli }),
  );
  return {
    name: 'docker',
    driver,
    ...
    async realize(spec) {
      ...
      return {
        containers: [{ role: 'agent', image: agent.image, env, mounts }],
        labels,
      };
    },
    failWith(message) {
  ...
  it(name, async () => {
    for (const harness of harnesses) {
      try {
        ...
        throw new Error(
          `[${harness.name}] ${error instanceof Error ? error.message : String(error)}`,
          {
            cause: error,
          },
        );
...
describe('conformance: spec realization fidelity', () => {
  eachDriver(
    'realizes the agent image and env exactly as declared',
    async (h) => {
  ...
  eachDriver(
    'realizes every declared agent mount, with its mode, and no others',
    async (h) => {
      const realized = await h.realize(fixtureSpec());
      const agent = realized.containers.find((c) => c.role === 'agent')!;
  ...
  eachDriver(
    'never places identity material in the agent container',
    async (h) => {
      const realized = await h.realize(fixtureSpec());
      const agent = realized.containers.find((c) => c.role === 'agent')!;

      expect(
        agent.mounts.some((m) => m.hostPath.includes('session-materials')),
      ).toBe(false);
  ...
  eachDriver(
    'carries the group-folder label byte-identical on every driver (D9)',
    async (h) => {
...
describe('conformance: mount and env policy', () => {
  const rejections: Array<[string, (spec: SessionSpec) => void]> = [
    ...
    [
      'a secret-shaped env key',
      (spec) => (spec.containers[0].env.SERVICE_TOKEN = 'shhh'),
    ],
  ];
  ...
  for (const [what, mutate] of rejections) {
    eachDriver(`rejects ${what} before allocating anything`, async (h) => {
      ...
      mutate(spec);
      await expect(h.driver.prepare(spec)).rejects.toMatchObject({
        kind: 'denied-by-policy',
        retryable: false,
      });
      expect(
        h.cli.calls.some(
          (c) => c.args[0] === 'create' && !c.args.includes('--dry-run=server'),
        ),
      ).toBe(false);
    });
  }
...
describe('conformance: the multi-container contract (validateSpec is the shared layer)', () => {
  ...
  it('accepts the overlay-composed auxiliary container, its identity material, and its trust anchor', () => {
    ...
    expect(() =>
      validateSpec(fixtureSpecWithAux(), FIXTURE_POLICY),
    ).not.toThrow();
  });
  ...
  it('refuses identity material that does not live under the material root', () => {
    ...
    expect(() => validateSpec(spec, FIXTURE_POLICY)).toThrow(
      /denied-by-policy/,
    );
  });
  ...
  it('keeps every auxiliary mount out of the agent container', () => {
    ...
    expect(
      agent.mounts
        ...
        .some((p) => p.startsWith('/run/session/')),
    ).toBe(false);
    expect(() => validateSpec(spec, FIXTURE_POLICY)).not.toThrow();
  });

  eachDriver(
    'a driver that does not manage auxiliary containers refuses the spec whole',
    async (h) => {
      ...
      if (h.driver.capabilities().auxiliaryContainers) {
        await expect(
          h.driver.prepare(fixtureSpecWithAux()),
...
describe('conformance: the isolation-tier rule (validateSpec is the shared layer)', () => {
  ...
  it('accepts any tier the capabilities declare', () => {
    ...
    spec.runtimeTier = 'vm';
    expect(() =>
      validateSpec(spec, FIXTURE_POLICY, capabilities(['container', 'vm'])),
    ).not.toThrow();
  ...
  eachDriver(
    'a driver validates the tier against its own declared capabilities',
    async (h) => {
      const spec = fixtureSpec();
      spec.runtimeTier = 'vm';
...
describe('conformance: admission judges the path the runtime mounts', () => {
  eachDriver(
    'refuses a mount whose path escapes its root lexically',
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].mounts.push({
  ...
  eachDriver(
    'refuses a relative hostPath (a named volume in Docker vocabulary, not a bind)',
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].mounts.push({
  ...
  eachDriver(
    "refuses another group's folder claimed as this session's group state",
    async (h) => {
  ...
  eachDriver(
    "still accepts this group's own folder as group state",
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].mounts.push({
        class: 'group-state',
  ...
  eachDriver(
    'refuses a spec without exactly one agent container',
    async (h) => {
      const zero = fixtureSpec();
      zero.containers = [];
      await expect(h.driver.prepare(zero)).rejects.toMatchObject({
        kind: 'spec-invalid',
      });
      const two = fixtureSpec();
      two.containers = [...two.containers, { ...two.containers[0] }];
      await expect(h.driver.prepare(two)).rejects.toMatchObject({
        kind: 'spec-invalid',
      });
    },
  );
...
describe('conformance: the contributed-env lane (provenance as structure)', () => {
  eachDriver(
    'accepts a credential-NAMED key on the contributed lane',
    async (h) => {
  ...
  eachDriver(
    'the contributed lane wins a key collision with composed env',
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].contributedEnv = {
        HTTPS_PROXY: 'http://gateway.internal:15001',
      };
      const realized = await h.realize(spec);
      const agent = realized.containers.find((c) => c.role === 'agent')!;
  ...
  eachDriver(
    'still refuses a credential VALUE on the contributed lane',
    async (h) => {
      // The lane exempts names, never bytes: real material rides mounts by
      // reference, from every contributor.
      const spec = fixtureSpec();
      spec.containers[0].contributedEnv = {
        UPSTREAM: 'sk-ant-api03-AAAAAAAAAAAAAAAAAAAAAAAA',
      };
      await expect(h.driver.prepare(spec)).rejects.toMatchObject({
        kind: 'denied-by-policy',
      });
...
describe('conformance: a class that grants cannot be claimed by a path that has not earned it', () => {
  ...
  eachDriver(
    'refuses a session private key relabelled as an allowlisted extra on the agent',
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].mounts.push({
  ...
  eachDriver(
    'refuses a writable runner source relabelled as an allowlisted extra',
    async (h) => {
      const spec = fixtureSpec();
      spec.containers[0].mounts.push({
  ...
  eachDriver(
    'accepts the stamped plugins tree as a read-only install surface',
    async (h) => {
  ...
  eachDriver(
    'refuses a stamped plugins tree relabelled as an allowlisted extra',
    async (h) => {
  ...
  eachDriver(
    "refuses another group's plugins tree claimed as this session's install surface",
    async (h) => {
  ...
  eachDriver(
    'still accepts an operator-configured read-write mount outside those roots',
    async (h) => {
...
describe('conformance: the no-secrets rule measures values, not names', () => {
  ...
  for (const [key, value] of leaks) {
    eachDriver(
      `refuses a credential in ${key}, whatever the key is called`,
      async (h) => {
        const spec = fixtureSpec();
  ...
  const legitimate: Array<[string, string]> = [
    ...
    [
      ...
      'sha256:9624d7f2e0b1c3a45566778899aabbccddeeff00112233445566778899aabbcc',
    ],
    ['SESSION_ID', 'a1b2c3d4-5566-7788-99aa-bbccddeeff00'],
  ];
  for (const [key, value] of legitimate) {
    eachDriver(
      `accepts ${key}, which is configuration and not a credential`,
      async (h) => {
        const spec = fixtureSpec();
        spec.containers[0].env[key] = value;
        await expect(h.driver.prepare(spec)).resolves.toBeDefined();
...
describe('conformance: failure taxonomy', () => {
  ...
  eachDriver(
    'maps an unreachable runtime to runtime-unavailable',
    async (h) => {
  ...
  eachDriver('never lets a raw runtime error cross the seam', async (h) => {
    h.failWith(
      '/install/data/session-materials/c/session-key.pem: permission denied',
    );
    await expect(h.driver.prepare(fixtureSpec())).rejects.toThrow(
      /^session realization failed: /,
    );
  });
});
...
describe('conformance: lifecycle', () => {
  ...
  eachDriver(
    'fires at most one terminal event, however many hints the stream carries',
    async (h) => {
  ...
  eachDriver(
    'stop is full teardown and reports no terminal event',
    async (h) => {
  ...
  eachDriver('reconstructs adopted handles from labels alone', async (h) => {
    ...
    expect(snapshots[0].handle.key).toEqual({
      installSlug: 'spike',
      agentGroupId: 'g1',
      sessionId: 's1',
    });
  ...
  eachDriver(
    'lists corpses honestly: a self-exited runtime is terminal, never dressed up as live',
    async (h) => {
...
describe('conformance: the attach surface describes, never execs', () => {
  eachDriver(
    'hands back a runnable argv that carries the client command',
    async (h) => {
      const handle = await h.driver.prepare(fixtureSpec());
