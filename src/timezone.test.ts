...
describe('parseZonedToUtc', () => {
  const iso = (s: string, tz: string): string =>
    parseZonedToUtc(s, tz).toISOString();

  it('reads a naive timestamp as wall-clock in a fixed-offset zone', () => {
    expect(iso('2026-06-20T09:00:00', 'Asia/Tokyo')).toBe(
  ...
  it('applies the correct seasonal offset (DST honored)', () => {
    ...
    expect(iso('2026-07-01T12:00:00', 'America/New_York')).toBe(
      '2026-07-01T16:00:00.000Z',
    ); // EDT -4
    expect(iso('2026-01-01T12:00:00', 'America/New_York')).toBe(
      '2026-01-01T17:00:00.000Z',
    ); // EST -5
  });

  it('passes a trailing-Z timestamp through unchanged', () => {
    expect(iso('2026-06-20T09:00:00Z', 'Asia/Tokyo')).toBe(
      '2026-06-20T09:00:00.000Z',
    );
  });

  it('passes an explicit offset through', () => {
    expect(iso('2026-06-20T09:00:00+02:00', 'Asia/Tokyo')).toBe(
      '2026-06-20T07:00:00.000Z',
    );
  });

  it('falls back to UTC for an invalid zone', () => {
    expect(iso('2026-06-20T09:00:00', 'Not/AZone')).toBe(
      '2026-06-20T09:00:00.000Z',
    );
  });
});
