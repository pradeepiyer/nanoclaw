...
import {
  createDbDriver,
  registerDbDriver,
  resetDbDriverRegistryForTesting,
...
describe('central DB driver registry', () => {
  it('fails closed when composition did not register a driver', async () => {
    await expect(createDbDriver(config, { role: 'runtime' })).rejects.toThrow(
      'No central DB driver registered',
    );
  });
  ...
  it('creates the registered driver and forwards role/config', async () => {
    ...
    await expect(createDbDriver(config, { role: 'host' })).resolves.toBe(
      driver,
    );
  ...
  it('rejects duplicate registration', () => {
    ...
    expect(() => registerDbDriver(() => driver)).toThrow(
      'Central DB driver already registered',
    );
  });
  ...
  it('rejects late registration after the first resolution attempt', async () => {
    ...
    expect(() => registerDbDriver(() => driver)).toThrow(
      'Central DB driver registration is closed',
    );
  });
});
