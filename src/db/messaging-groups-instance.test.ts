...
function mg(
  overrides: Partial<MessagingGroup> & { id: string },
): MessagingGroup {
  return {
    channel_type: 'slack',
...
describe('migration 016 — fresh DB', () => {
  ...
  it('createMessagingGroup without instance stamps instance = channel_type', async () => {
    ...
    const row = sqliteRaw(getDb())
      .prepare("SELECT instance FROM messaging_groups WHERE id = 'mg-default'")
      .get() as {
      instance: string;
    };
  ...
  it('allows sibling instances on the same (channel_type, platform_id)', async () => {
    ...
    await createMessagingGroup(
      mg({ id: 'mg-tester', instance: 'slack-tester' }),
    );
    const count = sqliteRaw(getDb())
      .prepare('SELECT COUNT(*) AS c FROM messaging_groups')
      .get() as { c: number };
  ...
  it('rejects a duplicate (channel_type, platform_id, instance) triple', async () => {
    ...
    await expect(
      createMessagingGroup(mg({ id: 'mg-b', instance: 'slack-tester' })),
    ).rejects.toThrow();
  });
...
describe('migration 016 — wired legacy DB upgrade (the FK recreate arm)', () => {
  it('recreates messaging_groups under FK children without violations and backfills instance', async () => {
    ...
    sqliteRaw(db)
      .prepare(
        "INSERT INTO agent_groups (id, name, folder, created_at) VALUES ('ag-1', 'A', 'a', ?)",
      )
      .run(now());
    sqliteRaw(db)
      .prepare(
    ...
    const row = sqliteRaw(db)
      .prepare("SELECT instance FROM messaging_groups WHERE id = 'mg-1'")
      .get() as {
      instance: string;
    };
    ...
    expect(
      sqliteRaw(db)
        .prepare(
          "SELECT COUNT(*) AS c FROM messaging_group_agents WHERE messaging_group_id = 'mg-1'",
        )
        .get(),
    ).toEqual({ c: 1 });
    expect(
      sqliteRaw(db)
        .prepare(
          "SELECT COUNT(*) AS c FROM sessions WHERE messaging_group_id = 'mg-1'",
        )
        .get(),
    ).toEqual({ c: 1 });
  ...
  it('tolerates pre-existing FK orphans: the migration still applies (no boot crash-loop)', async () => {
    ...
    sqliteRaw(db)
      .prepare(
        "INSERT INTO users (id, kind, created_at) VALUES ('slack:U1', 'slack', ?)",
      )
      .run(now());
  ...
  it('still rejects a migration that ITSELF introduces FK violations', async () => {
    ...
    const rogue: Migration = {
      ...
      up: (d) => {
        d.prepare(
          "INSERT INTO users (id, kind, created_at) VALUES ('slack:U-rogue', 'slack', datetime('now'))",
        ).run();
        d.prepare(
          `INSERT INTO user_dms (user_id, channel_type, messaging_group_id, resolved_at)
    ...
    await expect(runMigrations(db, [...migrations, rogue])).rejects.toThrow(
      /left FK violations/,
    );
    ...
    expect(
      sqliteRaw(db)
        .prepare(
          "SELECT 1 FROM schema_version WHERE name = 'test-rogue-fk-violation'",
        )
        .get(),
    ).toBeUndefined();
    expect(sqliteRaw(db).pragma('foreign_key_check')).toEqual([]);
  ...
  it('is idempotent — re-running the full barrel is a no-op', async () => {
    ...
    await expect(runMigrations(db)).resolves.toBeUndefined();
    const row = sqliteRaw(db)
      .prepare("SELECT instance FROM messaging_groups WHERE id = 'mg-keep'")
      .get() as {
      instance: string;
    };
...
describe('lookup asymmetry — inbound exact-only vs outbound default-first', () => {
  beforeEach(async () => {
    ...
    // mg-default.
    await createMessagingGroup(
      mg({ id: 'mg-tester', instance: 'alpha-tester' }),
    );
    await createMessagingGroup(mg({ id: 'mg-default' }));
  });
  ...
  it('getMessagingGroupWithAgentCount with an unknown instance returns null (no-hijack rule)', async () => {
    expect(
      await getMessagingGroupWithAgentCount(
        'slack',
        'slack:C1',
        'slack-unknown',
      ),
  ...
  it('getMessagingGroupByPlatform with explicit instance is exact', async () => {
    expect(
      (await getMessagingGroupByPlatform('slack', 'slack:C1', 'alpha-tester'))!
        .id,
    ).toBe('mg-tester');
    expect(
      await getMessagingGroupByPlatform('slack', 'slack:C1', 'slack-unknown'),
    ).toBeUndefined();
  });
  ...
  it('getMessagingGroupByPlatform falls back deterministically when only named instances exist', async () => {
    const db = getDb();
    sqliteRaw(db)
      .prepare("DELETE FROM messaging_groups WHERE id = 'mg-default'")
      .run();
