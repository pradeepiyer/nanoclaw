...
export async function findSessionByAgentGroup(
  agentGroupId: string,
): Promise<Session | undefined> {
  return getDb().get<Session>(
...
export async function getSessionsByAgentGroup(
  agentGroupId: string,
): Promise<Session[]> {
...
export async function findSystemSession(
  agentGroupId: string,
  threadId: string,
): Promise<Session | undefined> {
  return getDb().get<Session>(
...
export function isTaskThread(threadId: string | null): boolean {
  return (
    threadId === TASKS_SYSTEM_THREAD_ID ||
    (threadId?.startsWith(`${TASKS_SYSTEM_THREAD_ID}:`) ?? false)
  );
}

/** Task sessions for a group — active by default, optionally including closed history. */
export async function findTaskSessions(
  agentGroupId: string,
  includeClosed = false,
): Promise<Session[]> {
  return getDb().all<Session>(
...
export async function getPendingQuestion(
  ...
  >('SELECT * FROM pending_questions WHERE question_id = ?', questionId);
 ...
 */
export async function markApprovalAwaitingReason(
  approvalId: string,
  expiresAt: string,
): Promise<boolean> {
  const result = await getDb().run(
...
export async function getExpiredAwaitingReasonApprovals(
  nowIso: string,
...
export async function getPendingApprovalsByAction(
  action: string,
): Promise<PendingApproval[]> {
  return getDb().all<PendingApproval>(
    'SELECT * FROM pending_approvals WHERE action = ?',
    action,
  );
}
