...
export function defineDriverConformance(
  name: string,
  factory: DriverConformanceFactory,
): void {
  describe(`${name} central DB driver conformance`, () => {
    ...
    beforeEach(async () => {
      ...
      await db.exec(
        'CREATE TABLE items (id TEXT PRIMARY KEY, value TEXT, n INTEGER NOT NULL)',
      );
    });
    ...
    it('supports positional and named parameters', async () => {
      await db.run(
        'INSERT INTO items (id, value, n) VALUES (?, ?, ?)',
        'positional',
        'first',
        1,
      );
      await db.run(
        'INSERT INTO items (id, value, n) VALUES (@id, @value, @n)',
        {
          id: 'named',
          value: 'second',
          n: 2,
    ...
    it('reports changed rows', async () => {
      ...
      expect(
        (await db.run('UPDATE items SET value = ? WHERE id = ?', 'set', 'one'))
          .changes,
      ).toBe(1);
      expect(
        (await db.run('DELETE FROM items WHERE id = ?', 'missing')).changes,
      ).toBe(0);
    });
    ...
    it('commits, rolls back, and uses nested savepoints', async () => {
      ...
      expect(
        await db.all<{ id: string }>('SELECT id FROM items ORDER BY n'),
      ).toEqual([{ id: 'outer' }, { id: 'after-savepoint' }]);
    });

    it('executes compound SQL and reports table existence', async () => {
      expect(await db.hasTable('extra')).toBe(false);
      await db.exec(
        "CREATE TABLE extra (id TEXT PRIMARY KEY); INSERT INTO extra (id) VALUES ('x');",
      );
    ...
    it('returns numeric aggregates and deterministic C-style ordering', async () => {
      ...
      expect(
        await db.all<{ value: string | null }>(
          'SELECT value FROM items ORDER BY (value IS NULL), value',
        ),
      ).toEqual([{ value: 'B' }, { value: 'a' }, { value: null }]);
    });
    ...
    it('compares ISO timestamps lexically and returns truthy SELECT 1 rows', async () => {
      ...
      expect(
        await db.all<{ id: string }>(
          'SELECT id FROM items WHERE value > ? ORDER BY value',
    ...
    it('watchdog rolls back and releases an outside waiter', async () => {
      ...
      await waiting;
      release.resolve();
      expect(await db.all<{ id: string }>('SELECT id FROM items')).toEqual([
        { id: 'after' },
      ]);
    });
    ...
    it('cannot let a timed-out nested continuation roll back the next transaction', async () => {
      ...
      expect(
        await db.all<{ id: string }>('SELECT id FROM items ORDER BY n'),
      ).toEqual([{ id: 'current-outer' }, { id: 'current-inner' }]);
    });
  });
}
