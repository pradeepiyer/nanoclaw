 ...
 */
export function prepareTemplateTasks(
  tasks: TemplateTask[],
  timezone: string,
): Map<string, PreparedScheduledTask> {
  const slugs = new Map<string, string>();
  for (const task of tasks) {
    const slug = taskNameSlug(task.name);
    if (!slug) {
      throw new Error(
        `Template task ${task.source}: name "${task.name}" produces an empty id slug; rename it`,
      );
    }
    const clash = slugs.get(slug);
    if (clash !== undefined) {
      throw new Error(
        `Template tasks "${clash}" and "${task.name}" collide on id slug "${slug}"; rename one`,
      );
    }
  ...
  return new Map(
    tasks.map((task) => {
      try {
        ...
        throw new Error(`Invalid template task ${task.source}: ${message}`, {
          cause: err,
        });
      }
    }),
...
function parseTaskFile(
  ...
  const lines = fs
    .readFileSync(path.join(tasksDir, file), 'utf-8')
    .split(/\r?\n/);
  if (lines[0] !== '---')
    throw new Error(`Template task ${source} must start with --- frontmatter`);
  const closing = lines.indexOf('---', 1);
  if (closing === -1)
    throw new Error(`Template task ${source} is missing the closing ---`);
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    throw new Error(
      `Template task ${source} has invalid YAML frontmatter: ${message}`,
      { cause: err },
    );
  }
  if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) {
    throw new Error(
      `Template task ${source} frontmatter must be a YAML mapping`,
    );
  }
  const unknownFields = Object.keys(metadata).filter(
    (key) => key !== 'schedule' && key !== 'script',
  );
  if (unknownFields.length > 0) {
    throw new Error(
      `Template task ${source} frontmatter accepts only schedule and script`,
    );
  }
  ...
  if (typeof scheduleValue !== 'string' || !scheduleValue.trim()) {
    throw new Error(
      `Template task ${source} schedule must be a nonempty string`,
    );
  }
