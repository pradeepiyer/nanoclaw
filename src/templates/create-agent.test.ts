...
describe('createAgentFromTemplate', () => {
  ...
  it('copies template skills into the group-private Claude-plane skills dir', async () => {
    const { group: g } = await createAgentFromTemplate('sales/sdr', {
      name: 'SDR Skills',
    });
  ...
  it('copies the whole plugin into plugins/<name> and provisions plugin-data/<name>', async () => {
    ...
    expect(fs.existsSync(path.join(pluginDir, 'TROUBLESHOOTING.md'))).toBe(
      true,
    );
    expect(fs.existsSync(path.join(pluginDir, 'plugin.json'))).toBe(true);
    expect(
      fs.existsSync(path.join(pluginDir, 'skills', 'widget', 'SKILL.md')),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(GROUPS_DIR, g.folder, 'plugin-data', 'sdr')),
    ).toBe(true);
    ...
    expect(
      fs.existsSync(
        path.join(GROUPS_DIR, g.folder, 'plugin-data', 'sdr', 'state'),
      ),
    ).toBe(true);
  });
  ...
  it('records the ownership marker on every server and pluginRoot on stdio only', async () => {
    ...
    expect(servers.hubspot).toMatchObject({
      command: 'npx',
  ...
  it('refuses a template whose task names collide on the truncated id slug', async () => {
    ...
    await expect(
      createAgentFromTemplate('sales/sdr', { name: 'SDR Collide' }),
  ...
  it('derives the display name: explicit option → extension agentName → folder leaf', async () => {
    ...
    writeTemplate({
      extensions: {
        [NANOCLAW_EXTENSION_NS]: { agentName: 'Sales Development Rep' },
      },
    });
    ...
    fs.rmSync(TPL, { recursive: true, force: true });
    writeTemplate({
      extensions: { [NANOCLAW_EXTENSION_NS]: { agentName: 'Ignored' } },
    });
    const { group: explicit } = await createAgentFromTemplate('sales/sdr', {
      name: 'Chosen',
    });
  ...
  it('stamps a persona-less conformant plugin: no prepend file, group still complete', async () => {
    ...
    fs.writeFileSync(
      path.join(TPL, 'plugin.json'),
      JSON.stringify({ $schema: PLUGIN_SCHEMA_URL, name: 'greeter' }),
    );
    fs.writeFileSync(
      path.join(TPL, 'skills', 'greet', 'SKILL.md'),
      '---\nname: greet\ndescription: says hi\n---\n',
    );
    ...
    expect(
      fs.existsSync(path.join(GROUPS_DIR, g.folder, PERSONA_PREPEND_FILE)),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(GROUPS_DIR, g.folder, 'plugins', 'greeter', 'plugin.json'),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it('surfaces the reader report to the caller', async () => {
    ...
    const { report } = await createAgentFromTemplate('sales/sdr', {
      name: 'SDR Report',
    });
    expect(report).toEqual([
  ...
  it('rejects a template containing a symlink before any group state is created', async () => {
    ...
    await expect(
      createAgentFromTemplate('sales/sdr', { name: 'Evil' }),
  ...
  it('creates template tasks paused through the normal isolated task-session path', async () => {
    ...
    const db = new Database(inboundDbPath(g.id, sessions[0].id), {
      readonly: true,
    });
    const row = db
      .prepare(
        "SELECT status, recurrence, process_after, content FROM messages_in WHERE kind = 'task'",
      )
      .get() as {
      status: string;
      recurrence: string;
    ...
    expect(
      fs.existsSync(
        path.join(GROUPS_DIR, g.folder, 'tasks', 'weekday-briefing.md'),
      ),
    ).toBe(false);
  });
  ...
  it('stamps a valid timezone onto the config row and grounds template task first runs in it', async () => {
    ...
    const row = db
      .prepare("SELECT process_after FROM messages_in WHERE kind = 'task'")
      .get() as {
      process_after: string;
    };
    db.close();
  ...
  it('forwards multiline scripts unchanged through the shared task creation path', async () => {
    ...
    const row = db
      .prepare(
        "SELECT status, recurrence, content FROM messages_in WHERE kind = 'task'",
      )
      .get() as {
      status: string;
      recurrence: string;
  ...
  it.each([
    ...
    'rejects %s before creating the agent group',
    async (_case, schedule, expected) => {
      writeTask('broken', schedule, 'Never created.');

      await expect(
        createAgentFromTemplate('sales/sdr', { name: 'Broken Tasks' }),
