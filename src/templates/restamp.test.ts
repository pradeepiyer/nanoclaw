...
function writeTemplateV1(): void {
  ...
  fs.mkdirSync(
    path.join(TPL, NANOCLAW_EXTENSION_NS, 'context', 'additional_context'),
    { recursive: true },
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(TPL, NANOCLAW_EXTENSION_NS, 'context', 'playbook.md'),
    '# Playbook v1\n',
  );
  fs.writeFileSync(
    path.join(
  ...
  fs.writeFileSync(
    path.join(TPL, 'TROUBLESHOOTING.md'),
    '# When things break v1\n',
  );
...
function writeTemplateV2(): void {
  ...
  fs.writeFileSync(
    path.join(TPL, NANOCLAW_EXTENSION_NS, 'context', 'guide.md'),
    '# Guide\n',
  );
  fs.writeFileSync(
    path.join(TPL, 'mcp.json'),
...
async function servers(
  ...
  return JSON.parse((await getContainerConfig(groupId))!.mcp_servers) as Record<
    string,
    McpServerConfig
  >;
}

async function liveTasks(groupId: string): Promise<
  Array<{
    series_id: string;
    status: string;
    recurrence: string;
...
> {
  ...
  for (const session of await findTaskSessions(groupId)) {
    const db = new Database(inboundDbPath(groupId, session.id), {
      readonly: true,
    });
    for (const row of db
      .prepare(
      ...
      rows.push({
        ...row,
        prompt: (JSON.parse(row.content) as { prompt: string }).prompt,
      });
    }
    db.close();
  }
...
async function allTaskSeries(groupId: string): Promise<string[]> {
  ...
  for (const session of await findTaskSessions(groupId)) {
    ...
    for (const row of db
      .prepare("SELECT DISTINCT series_id FROM messages_in WHERE kind = 'task'")
      .all() as Array<{
      series_id: string;
    }>) {
...
describe('plugin ownership marker', () => {
  it('stamps the plugin marker on every server and pluginRoot on stdio only', async () => {
    ...
    expect(s.hubspot).toMatchObject({
      plugin: 'sdr',
      pluginRoot: '/workspace/agent/plugins/sdr',
    });
...
describe('restampAgentFromTemplate', () => {
  it('throws when the group does not carry the plugin', async () => {
    ...
    await expect(
      restampAgentFromTemplate('sales/sdr', g.id, { apply: false }),
    ).rejects.toThrow(/does not carry plugin "sdr"/);
  });

  it('reports all-unchanged when the template did not change', async () => {
    const g = await stamp();
    const result = await restampAgentFromTemplate('sales/sdr', g.id, {
      apply: true,
    });
  ...
  it('treats an exec-bit-only template change as an update', async () => {
    ...
    expect(result.changes.find((c) => c.surface === 'plugin')?.action).toBe(
      'update',
    );
  });
  ...
  it('dry run reports every change but mutates nothing', async () => {
    ...
    expect(change(result.changes, 'persona', PERSONA_PREPEND_FILE).action).toBe(
      'update',
    );
    expect(change(result.changes, 'context', 'playbook.md').action).toBe(
      'update',
    );
    expect(
      change(result.changes, 'context', 'additional_context/faq.md').action,
    ).toBe('remove');
    ...
    expect(change(result.changes, 'mcp-server', 'hubspot').action).toBe(
      'update',
    );
    expect(change(result.changes, 'mcp-server', 'docs').action).toBe('remove');
    expect(change(result.changes, 'mcp-server', 'search').action).toBe(
      'create',
    );
    expect(change(result.changes, 'task', 'weekday-briefing').action).toBe(
      'update',
    );
    expect(change(result.changes, 'task', 'weekly-report').action).toBe(
      'create',
    );
    ...
    expect(
      fs.existsSync(path.join(groupDir, 'additional_context', 'faq.md')),
    ).toBe(true);
    ...
    expect(
      fs.readFileSync(
        path.join(groupDir, 'plugins', 'sdr', 'TROUBLESHOOTING.md'),
        'utf-8',
      ),
    ).toContain('v1');
  });
  ...
  it('applies the full update across every plugin-owned surface', async () => {
    ...
    fs.writeFileSync(
      overlaySkill(g.id, 'own'),
      '---\nname: own\ndescription: agent made\n---\n',
    );
    fs.writeFileSync(
      path.join(groupDir, 'plugin-data', 'sdr', 'state.json'),
      '{"seen": 3}\n',
    );
    ...
    expect(
      fs.readFileSync(path.join(groupDir, PERSONA_PREPEND_FILE), 'utf-8'),
    ).toBe('You are an SDR agent v2.\n');
    expect(fs.readFileSync(path.join(groupDir, 'playbook.md'), 'utf-8')).toBe(
      '# Playbook v2\n',
    );
    expect(
      fs.existsSync(path.join(groupDir, 'additional_context', 'faq.md')),
    ).toBe(false);
    expect(fs.readFileSync(path.join(groupDir, 'guide.md'), 'utf-8')).toBe(
      '# Guide\n',
    );
    // Skills overlay refreshed; agent-authored skill untouched.
    expect(fs.readFileSync(overlaySkill(g.id, 'widget'), 'utf-8')).toContain(
      'v2',
    );
    ...
    expect(fs.readFileSync(path.join(groupDir, 'notes.md'), 'utf-8')).toBe(
      'agent notes\n',
    );
    expect(
      fs.readFileSync(
  ...
  it('flags locally customized files and resets them on apply', async () => {
    ...
    fs.writeFileSync(
      ...
      'My hand-tuned persona.\n',
    );
    fs.writeFileSync(path.join(groupDir, 'playbook.md'), '# My playbook\n');
    fs.writeFileSync(
      overlaySkill(g.id, 'widget'),
      '---\nname: widget\ndescription: my tweak\n---\n',
    );
    writeTemplateV2();

    const plan = await restampAgentFromTemplate('sales/sdr', g.id, {
      apply: false,
    });
    expect(
      change(plan.changes, 'persona', PERSONA_PREPEND_FILE).customized,
    ).toBe(true);
    expect(change(plan.changes, 'context', 'playbook.md').customized).toBe(
      true,
    );
    expect(change(plan.changes, 'skill', 'widget').customized).toBe(true);
    // Unmodified surfaces are not flagged.
    expect(
      change(plan.changes, 'context', 'guide.md').customized,
    ).toBeUndefined();
  ...
  it('never clobbers a user-added server whose name a new plugin server takes', async () => {
    ...
    expect((await servers(g.id)).search).toEqual({
      type: 'http',
      url: 'https://mine.example.com/mcp',
    });
  });
  ...
  it('neutralizes planted file symlinks instead of writing through them', async () => {
    ...
    expect(
      fs.lstatSync(path.join(groupDir, PERSONA_PREPEND_FILE)).isSymbolicLink(),
    ).toBe(false);
    expect(
      fs.readFileSync(path.join(groupDir, PERSONA_PREPEND_FILE), 'utf-8'),
    ).toBe('You are an SDR agent v2.\n');
  ...
  it('skips instead of duplicating a series that is re-arming between runs', async () => {
    ...
    for (const session of await findTaskSessions(g.id)) {
      withInboundDb(g.id, session.id, (db) =>
        db
          .prepare(
            "UPDATE messages_in SET status = 'completed' WHERE series_id = ?",
          )
          .run(series),
      );
    }
    ...
    const seriesIds = (await allTaskSeries(g.id)).filter((id) =>
      id.startsWith('weekday-briefing-'),
    );
    expect(seriesIds).toEqual([series]);
    expect(
      (await liveTasks(g.id)).some((t) =>
        t.series_id.startsWith('weekly-report-'),
      ),
    ).toBe(true);
  });
  ...
  it('deletes a dropped series even while it is re-arming', async () => {
    ...
    writeTemplateV2();
    fs.rmSync(path.join(TPL, NANOCLAW_EXTENSION_NS, 'tasks'), {
      recursive: true,
    });
  ...
  it('updates the stamped series, not a newer user task with the same name', async () => {
    ...
    const mine = (
      await createScheduledTask(g.id, prepared, { status: 'pending' })
    ).row.seriesId!;
    writeTemplateV2();
    ...
    const tasks = await liveTasks(g.id);
    expect(tasks.find((t) => t.series_id === mine)!.prompt).toBe(
      'My own briefing.',
    );
    expect(tasks.find((t) => t.series_id === stamped)!.prompt).toBe(
      'Send the improved weekday briefing.',
    );
  });
  ...
  it('treats a rename that preserves the slug as an update, never remove+create', async () => {
    ...
    expect(
      result.changes.filter(
        (c) => c.surface === 'task' && c.action === 'remove',
      ),
    ).toEqual([]);
    const tasks = await liveTasks(g.id);
  ...
  it('rejects a template whose task names collide on the truncated id slug', async () => {
    ...
    await expect(
      restampAgentFromTemplate('sales/sdr', g.id, { apply: false }),
    ).rejects.toThrow(/collide on id slug/);
  });
