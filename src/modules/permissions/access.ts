...
export type AccessDecision =
  | {
      allowed: true;
      reason: 'owner' | 'global_admin' | 'admin_of_group' | 'member';
    }
  | { allowed: false; reason: 'unknown_user' | 'not_member' };

/** Can this user interact with this agent group? */
export async function canAccessAgentGroup(
  userId: string,
  agentGroupId: string,
  ...
  if (await isAdminOfAgentGroup(userId, agentGroupId))
    return { allowed: true, reason: 'admin_of_group' };
  if (await isMember(userId, agentGroupId))
