...
let agentDmOpenedHandler:
  | ((event: AgentDmOpenedEvent) => void | Promise<void>)
  | null = null;

export function setAgentDmOpenedHandler(
  fn: (event: AgentDmOpenedEvent) => void | Promise<void>,
): void {
...
function dispatchAgentDmOpened(event: AgentDmOpenedEvent): void {
  ...
  try {
    Promise.resolve(agentDmOpenedHandler(event)).catch((err) =>
      log.warn('Agent-DM opened handler failed', {
        channelId: event.channelId,
        err,
      }),
    );
  } catch (err) {
    log.warn('Agent-DM opened handler threw', {
      channelId: event.channelId,
      err,
    });
  }
...
const appContextCache = new Map<
  string,
  { entities: AppContextEntity[]; at: number }
>();
...
function appContextKey(
  ...
  const channel = channelId.includes(':')
    ? channelId.slice(channelId.lastIndexOf(':') + 1)
    : channelId;
...
export function appContextEntities(
  ...
  const direct =
    raw.entities ??
    (raw.context as Record<string, unknown> | undefined)?.entities;
 ...
 */
export function normalizeDmThreadId(
  threadId: string,
  messageId: string,
): string {
 ...
 */
export function setMembershipHandler(
  channelType: string,
  fn: MembershipHandler,
): void {
 ...
 */
export type BridgeInboundPolicy = (
  setup: ChannelSetup,
  instanceKey: string,
) => ChannelSetup | Promise<ChannelSetup>;
 ...
 */
export function registerBridgeInboundPolicy(
  channelType: string,
  wrap: BridgeInboundPolicy,
): void {
 ...
 */
export function buildTerminalApprovalCard({
  title,
  question,
  resolution,
...
function terminalApprovalMessage(spec: TerminalApprovalCard) {
  return {
    ...
    fallbackText: [spec.title, spec.question, spec.resolution]
      .filter(Boolean)
      .join('\n\n'),
  };
}
...
export function createChatSdkBridge(
  ...
  const bridge: ChannelAdapter = {
    ...
    async setup(hostConfig: ChannelSetup) {
      ...
      setupConfig = inboundPolicy
        ? await inboundPolicy(hostConfig, instanceKey)
        : hostConfig;
      ...
      state = new SqliteStateAdapter(
        config.instance && config.instance !== adapter.name
          ? config.instance
          : undefined,
      );
      ...
      chat.onAction(async (event) => {
        ...
        const resolution = actorName
          ? `${selectedLabel} by ${actorName}`
          : selectedLabel;
        try {
      ...
      if (gatewayAdapter.startGatewayListener) {
        ...
        log.info('Polling adapter: no webhook route registered', {
          adapter: adapter.name,
        });
      } else {
    ...
    async deliver(
      ...
      if (
        ...
        if (Array.isArray(cardSpec.children)) {
          for (const child of cardSpec.children) {
            if (typeof child === 'string' && child) {
              ...
              cardChildren.push(
                CardText((child as Record<string, string>).text),
              );
            }
          }
        }
        if (Array.isArray(cardSpec.actions)) {
          const linkButtons = (
            cardSpec.actions as Array<Record<string, unknown>>
          )
            .filter(
              (a) =>
                typeof a.url === 'string' &&
  ...
  if (event.type === 'GATEWAY_INTERACTION_CREATE' && event.data) {
    ...
    if (interaction.type === 3) {
      ...
      const render = questionId
        ? await resolveQuestionRender(questionId)
        : undefined;
