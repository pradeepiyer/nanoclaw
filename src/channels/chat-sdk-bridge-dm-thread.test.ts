...
describe('normalizeDmThreadId', () => {
  it('roots a top-level DM message on its own ts', () => {
    expect(normalizeDmThreadId('slack:D0123ABC:', '1724264405.531769')).toBe(
  ...
  it('leaves an already-threaded reply untouched', () => {
    ...
    ).toBe('slack:D0123ABC:1724264405.531769');
  });

  it('ignores encoded (non-ts) message ids', () => {
    expect(normalizeDmThreadId('slack:D0123ABC:', 'ephemeral:1724:url')).toBe(
      'slack:D0123ABC:',
    );
  });
