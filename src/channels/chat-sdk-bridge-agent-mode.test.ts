...
describe('app context cache', () => {
  it('caches per (instance, channel, user) and consumes single-shot', () => {
    cacheAppContext('slack-pixel', 'D0DM1', 'U1', [
      { type: 'channel', id: 'C0DESIGN' },
    ]);
    expect(takeAppContext('slack-pixel', 'D0DM1', 'U1')).toEqual([
      { type: 'channel', id: 'C0DESIGN' },
    ]);
  ...
  it('matches raw and platform-encoded channel ids (slack:D… vs D…)', () => {
    ...
    expect(takeAppContext('slack-pixel', 'slack:D0DM1', 'U1')).toEqual([
      { type: 'channel', id: 'C9' },
    ]);
  });
  ...
  it('is instance- and user-scoped', () => {
    ...
    expect(takeAppContext('slack-pixel', 'D1', 'U1')).toEqual([
      { type: 'channel', id: 'C1' },
    ]);
  });
  ...
  it('expires after the TTL (~5min)', () => {
    ...
    expect(
      takeAppContext('slack-pixel', 'D1', 'U1', t0 + 5 * 60 * 1000 + 1),
    ).toBeUndefined();
  });
  ...
  it('attachAppContext attaches { entities } once and never overwrites SDK-provided context', () => {
    ...
    expect(content.app_context).toEqual({
      entities: [{ type: 'channel', id: 'C1' }],
    });
    ...
    expect(preAttached.app_context).toEqual({
      entities: [{ type: 'list', id: 'L1' }],
    });
