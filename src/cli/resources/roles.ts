...
registerResource({
  ...
  columns: [
    {
      name: 'user_id',
      type: 'string',
      description: 'User receiving the role. Must exist in users table.',
    },
    {
      name: 'role',
      type: 'string',
      description:
        '"owner" has full control, always global. "admin" can manage groups and approve actions.',
      enum: ['owner', 'admin'],
    },
    {
      name: 'agent_group_id',
    ...
    },
    {
      name: 'granted_by',
      type: 'string',
      description: 'Who granted this role. Informational.',
    },
  ...
  customOperations: {
    grant: {
      access: 'approval',
      description:
        'Grant a role. Use --user, --role, and optionally --group for scoped admin.',
      handler: async (args) => {
        ...
        if (role === 'owner' && groupId)
          throw new Error('owner role is always global (do not pass --group)');
        getDb()
          .prepare(
