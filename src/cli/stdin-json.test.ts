...
describe('bounded stdin JSON', () => {
  it('merges one chunked JSON object into argv args', async () => {
    const result = await readStdinJsonArgs(
      stream('{"source":"stdin",', Buffer.from('"nested":{"enabled":true}}')),
      {
        name: 'example',
      },
    );
  ...
  it('accepts an object whose UTF-8 representation is exactly 64 KiB', async () => {
    ...
    expect((result.payload as string).length).toBe(
      MAX_STDIN_JSON_BYTES - envelopeBytes,
    );
  });
  ...
  it('measures multibyte input as UTF-8 bytes across chunk boundaries', async () => {
    ...
    const result = await readStdinJsonArgs(
      stream(source.subarray(0, 12), source.subarray(12)),
      {},
    );
  ...
  it('rejects malformed JSON', async () => {
    await expect(readStdinJsonArgs(stream('{"broken":'), {})).rejects.toThrow(
  ...
  it('rejects a key also supplied on argv', async () => {
    await expect(
      readStdinJsonArgs(stream('{"subject":"stdin-value"}'), {
        subject: 'argv-value',
      }),
    ).rejects.toThrow('--stdin-json key "subject" is also supplied on argv');
  });
  ...
  it('rejects a stdin key that aliases a hyphenated argv key after CLI normalization', async () => {
    await expect(
      ...
      '--stdin-json key "group_id" conflicts with argv key "group-id" after CLI key normalization',
    );
  });

  it('rejects two stdin keys that alias each other after CLI normalization', async () => {
    await expect(
      readStdinJsonArgs(stream('{"group-id":"first","group_id":"second"}'), {}),
    ).rejects.toThrow(
  ...
  it('rejects __proto__ before downstream argument normalization', async () => {
    await expect(
      readStdinJsonArgs(stream('{"__proto__":{"id":"task-123"}}'), {}),
    ).rejects.toThrow('--stdin-json key "__proto__" is not allowed');
  });
});
...
describe('host CLI flags', () => {
  it('keeps --json as output mode while excluding --stdin-json from request args', () => {
    expect(
      parseArgv(['groups', 'update', '--stdin-json', '--json', '--force']),
    ).toEqual({
