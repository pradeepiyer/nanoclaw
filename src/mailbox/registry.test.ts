...
describe('agent mailbox registry', () => {
  ...
  it('keeps the host entrypoint on the real composition barrel', () => {
    expect(
      fs.readFileSync(new URL('../modules/index.ts', import.meta.url), 'utf8'),
    ).toContain("import '../mailbox/compose.js';");
    expect(
      fs.readFileSync(new URL('../index.ts', import.meta.url), 'utf8'),
    ).toContain("import './modules/index.js';");
  });

  it('keeps session SQLite code inside the SQLite driver', () => {
    expect(fs.existsSync(new URL('../db/session-db.ts', import.meta.url))).toBe(
      false,
    );
    expect(
      fs.existsSync(new URL('./sqlite/session-db.ts', import.meta.url)),
    ).toBe(true);
  ...
  it('rejects two implementations for the same capability', () => {
    ...
    expect(() => registerAgentMailbox(fakeMailbox)).toThrow(
      'already registered',
    );
  });
