...
describe('filtered commands', () => {
  it('drops /start before it reaches the container', async () => {
    expect(await gateCommand('/start', 'telegram:1', 'ag-1')).toEqual({
      action: 'filter',
    });
  });

  it('drops /start regardless of sender', async () => {
    expect(await gateCommand('/start', null, 'ag-1')).toEqual({
      action: 'filter',
    });
  });
});
...
describe('admin gating goes through roles', () => {
  it('denies an admin command from a non-admin user', async () => {
    expect(await gateCommand('/clear', 'telegram:nobody', 'ag-1')).toEqual({
      action: 'deny',
      command: '/clear',
    });
  });
  ...
  it('denies an admin command with no sender', async () => {
    expect(await gateCommand('/clear', null, 'ag-1')).toEqual({
      action: 'deny',
      command: '/clear',
    });
  });
  ...
  it('allows an admin command from an owner', async () => {
    ...
    expect(await gateCommand('/clear', 'telegram:owner', 'ag-1')).toEqual({
      action: 'pass',
    });
  });
  ...
  it('allows an admin command from a scoped admin of the group', async () => {
    ...
    expect(await gateCommand('/clear', 'telegram:admin', 'ag-1')).toEqual({
      action: 'pass',
    });
    expect(await gateCommand('/clear', 'telegram:admin', 'ag-2')).toEqual({
      action: 'deny',
      command: '/clear',
    });
  });
...
describe('normal messages pass through', () => {
  it('passes a plain message', async () => {
    expect(await gateCommand('hello there', 'telegram:1', 'ag-1')).toEqual({
      action: 'pass',
    });
  });

  it('passes an unknown slash command', async () => {
    expect(await gateCommand('/whatever', 'telegram:1', 'ag-1')).toEqual({
      action: 'pass',
    });
  });
});
